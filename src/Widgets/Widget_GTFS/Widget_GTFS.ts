...
class Gtfs extends Widget {
  ...
  public Load(): void {
    ...
    tableHeaderRouteInfo.innerHTML = `<h3>${
      (<GtfsWidgetSettings>this.WidgetSettings).Start
    }&nbsp;<i class="fas fa-angle-right"></i>&nbsp;${
      (<GtfsWidgetSettings>this.WidgetSettings).Destination
    }</h3>`;

    this.initGtfsData().then(() => {
      console.log(`Widget GTFS -> Data fetched`);

      let tripsToShow = this.getRoutes(
        (<GtfsWidgetSettings>this.WidgetSettings).Start,
        (<GtfsWidgetSettings>this.WidgetSettings).Destination
      );
      ...
      tripsToShow.forEach((tripToShow: any, key: any) => {
        ...
        tripToShow.forEach((stop: any) => {
          tdRoute.innerHTML += `<div>${
            this.FormatTime(stop.departure_time.getHours()) +
            ":" +
            this.FormatTime(stop.departure_time.getMinutes())
          }&nbsp;-&nbsp;${
            this.stops.filter((x) => x.stop_id === stop.stop_id)[0].stop_name
          } </div>`;
        });
      });
  ...
  public getRoutes(start: string, destination: string): any {
    ...
    let stopStartIds = this.stops
      .filter((x) => x.stop_name === start)
      .map((x) => x.stop_id);
    let stopDestinationIds = this.stops
      .filter((x) => x.stop_name === destination)
      .map((x) => x.stop_id);

    // Haltestellen Halte nach Fahrten gruppieren
    let now = new Date();
    let stop_times = this.groupBy(
      this.stop_times.filter((x) => x.departure_time > now),
      (stopTimes: GtfsClasses.StopTimes) => stopTimes.trip_id
    );
  ...
  public AddTrip(
    groupedStopTimes: Map<Number, GtfsClasses.StopTimes[]>,
    linie_trip: Map<any, any>,
    startStopIds: Number[],
    destinationStopIds: Number[]
  ): void {
    groupedStopTimes.forEach((groupedStopTime, tripId: Number) => {
      let startStop = groupedStopTime.filter((x) =>
        startStopIds.includes(x.stop_id)
      )[0];
      if (startStop === undefined) {
        return;
      }
      let destinationStop = groupedStopTime.filter(
        (x) =>
          destinationStopIds.includes(x.stop_id) &&
          x.stop_sequence > startStop.stop_sequence
      )[0];

      if (startStop !== undefined && destinationStop !== undefined) {
        let routeId = this.trips.filter((x) => x.trip_id === tripId)[0]
          .route_id;
  ...
  public async initGtfsData(): Promise<void> {
    // agency
    this.agency = this.MapCsv<GtfsClasses.Agency[]>(
      GtfsClasses.GtfsTypeMap().get("agency"),
      await this.fetchGtfsData("agency")
    )[0];

    // calendar_dates
    this.calendar_dates = this.MapCsv<GtfsClasses.CalendarDates[]>(
      GtfsClasses.GtfsTypeMap().get("calendar_dates"),
      await this.fetchGtfsData("calendar_dates")
    );

    // feed_info
    this.feed_info = this.MapCsv<GtfsClasses.FeedInfo[]>(
      GtfsClasses.GtfsTypeMap().get("feed_info"),
      await this.fetchGtfsData("feed_info")
    );

    // routes
    this.routes = this.MapCsv<GtfsClasses.Routes[]>(
      GtfsClasses.GtfsTypeMap().get("routes"),
      await this.fetchGtfsData("routes")
    );

    // stops
    this.stops = this.MapCsv<GtfsClasses.Stops[]>(
      GtfsClasses.GtfsTypeMap().get("stops"),
      await this.fetchGtfsData("stops")
    );

    // stop_times
    this.stop_times = this.MapCsv<GtfsClasses.StopTimes[]>(
      GtfsClasses.GtfsTypeMap().get("stop_times"),
      await this.fetchGtfsData("stop_times")
    );

    // trips
    this.trips = this.MapCsv<GtfsClasses.Trips[]>(
      GtfsClasses.GtfsTypeMap().get("trips"),
      await this.fetchGtfsData("trips")
    );
  }
  ...
  public fetchGtfsData(dataType: string): Promise<string> {
    return new Promise((resolve, reject) => {
      fetch(
        `${url.resolve(
          (<GtfsWidgetSettings>this.WidgetSettings).GtfsBaseAddress,
          dataType + ".txt"
        )}`
      )
        .then((res) => res.text())
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      for (let f = 0; f < values.length; f++) {
        ...
        try {
          switch (typeDefinition[headers[f]]) {
            ...
            case Date:
              if (values[f].includes(":")) {
                ...
                parsedValue = new Date(
                  `${values[f].substr(4, 2)}.${values[f].substr(6, 2)}.${values[
                    f
                  ].substr(0, 4)}`
                );
              }
          ...
          console.error(
            `Error parsing ${values[f]} as ${
              typeDefinition[headers[f]]
            } for header ${headers[f]}`
          );
          throw err;
        }
...
export = (
  da: Services.DataAdapter,
  ms: Services.MirrorService,
  logger: Services.LoggingService
) => new Gtfs(da, ms, logger);

...
export class Todoist extends Widget {
  ...
  public Load(): void {
    ...
    this.showWidget(
      (<TodoistWidgetSettings>this.WidgetSettings).Project,
      (<TodoistWidgetSettings>this.WidgetSettings).ApiKey
    );
  }
  ...
  private showWidget(project: string, apiKey: string): void {
    ...
    this.fetchTasks(project, apiKey).then((tasks: any) => {
      ...
      let taskSpanMap: Map<string, HTMLSpanElement> = new Map<
        string,
        HTMLSpanElement
      >();
      ...
      for (let index = 0; index < tasks.length; index++) {
        ...
        if (tasks[index].due !== undefined) {
          ...
          htmlText += `&nbsp;&nbsp;&nbsp;<i style="font-size: 13px; font-style: normal;">${
            this.GetWeekDay(dueDate.getDay()) +
            " " +
            this.FormatTime(dueDate.getDate()) +
            "." +
            this.FormatTime(dueDate.getMonth() + 1) +
            "." +
            dueDate.getFullYear()
          }</i>`;
        } else {
        ...
        taskSpanMap
          .get(tasks[index].parent_id)
          .appendChild(taskSpanMap.get(tasks[index].id));
      }
    });
  }
    ...
    try {
      ...
      for (let index = 0; index < projects.length; index++) {
        if (projects[index].name == projectName) {
          returnValue = await this.get(
            `https://api.todoist.com/rest/v1/tasks?project_id=${projects[index].id}`,
            { Authorization: `Bearer ${apiKey}` }
          );

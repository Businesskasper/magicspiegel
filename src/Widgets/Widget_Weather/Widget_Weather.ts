...
export class Weather extends Widget {
  ...
  public Load(): void {
    ...
    this.fetchWeatherData(
      (<WeatherWidgetSettings>this.WidgetSettings).CityName,
      (<WeatherWidgetSettings>this.WidgetSettings).OpenWeatherMapApiKey
    )
      .then((weatherData: WeatherData[]) => {
        //Build Table
        containerDiv.appendChild(this.buildTable(weatherData));
      })
      .catch((err) => {
  ...
  private buildTable(weatherData: WeatherData[]): HTMLTableElement {
    ...
    let groupedWeatherData: Map<string, WeatherData[]> = this.groupBy(
      weatherData,
      (weatherDay: WeatherData) => weatherDay.DayOfWeek
    );
    ...
    //Build table body
    Array.from(["06:00", "09:00", "12:00", "18:00", "21:00"]).forEach(
      (timeValue) => {
        //Build time cell
        ...
        groupedWeatherData.forEach((weatherDay: WeatherData[]) => {
          ...
          weatherDay.forEach((weatherDayValue: WeatherData) => {
            if (weatherDayValue.Time === timeValue) {
              tableData.innerHTML = `${this.getWeatherSymbol(
                weatherDayValue.WeatherCode
              )}<br>${this.getTemperature(weatherDayValue.Temperature)}°`;
            }
          });
        });
      }
